...
for i = 1:1:n
    ...
    E(i) = hafit.E;
    s_jnt = s(n_glim);
    i_jnt_g = exp(-1/3*Rg(i)^2*s_jnt^2);
    i_jnt_ha = A(i)*s_jnt^-4 + E(i);
    ...
    loglog(s, i_s)
    hold on
    loglog(s(1:n_glim), exp(-1/3*Rg_mod(i)^2.*s(1:n_glim).^2))
    loglog(s_ha, A(i)*s_ha.^-4 + E(i))
    loglog([s_jnt], [i_jnt_g], 'o');
    hold off
    axis([0, 5, 0, 1])
...
i_jnt_ha = A_final*s_jnt^-4 + E_final;
contrat = i_jnt_g/i_jnt_ha;
A_final = contrat*A_final;
E_final = contrat*E_final;
%% generate final fit plot
plot(s(1:n_glim), exp(-1/3*Rg_final^2.*s(1:n_glim).^2))
hold on
plot(s_ha, A_final*s_ha.^-4 + E_final)
hold off
axis([0, 5, 0, 1])
%% output parameters
disp([Rg_final, A_final, E_final]);
